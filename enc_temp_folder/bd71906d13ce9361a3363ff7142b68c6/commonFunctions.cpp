...
namespace commonFunctions
{
		...
		if (areYouSure == 0) // Run sequence if user responds ok
		{
			for (UINT configInc = 0; configInc < configSequence.size(); configInc++)
			{
				...
				{	
					...
					if (pyStarted == 0)
					{
						multiExpInput->mainWin->getComm()->sendStatus("\r\nPython start file created. Waiting for response...\r\n");
					...
					{
						multiExpInput->mainWin->getComm()->sendStatus("\r\nFAILED to create Python start file.\r\n");
					}
				}
			}
		}
				...
				while (true)
				{
					...
					case FILE_ACTION_RENAMED_NEW_NAME:
					{
						...
						if (fileName == "pyFinish.txt")
						{
							...
							mainWin->getComm()->sendStatus("pyFinish.txt detected!");
						}
					}
					break;
	...
	void updateGlobalVars(MainWindow* mainWin, AuxiliaryWindow* auxWin)
	{
		...
		if (pyFinishFile.is_open())
		{
			...
			try
			{
				if (auxWin->globalVariables.checkVariableExists(variableToChange))
				{
					...
					mainWin->getComm()->sendStatus("\r\nUpdated " + variableToChange + " from " + std::to_string(currentValue) + " to " + changeValueStr +".\r\n");
				}
				else
				{
		...
		mainWin->getComm()->sendStatus("Python analysis finished!\r\n");
	}
