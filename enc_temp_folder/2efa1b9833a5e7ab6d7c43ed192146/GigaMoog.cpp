		...
		for (int channel = 0; channel < 16; channel++) {//TODO: 16 could be changed to 64 if using more tones for rearrangement
			if (channel < nx) {
				...
				Message m = Message::make().destination(MessageDestination::KA007)
					...
					.frequencyMHz(freq).amplitudePercent(amp).phaseDegrees(phase).instantFTW(1).ATWIncr(ampStepMag).stepSequenceID(singleStepID).FTWIncr(0).phaseJump(1);;
				ms.enqueue(m);
			}
			else //populate extra channels with null moves.
			{
				Message m = Message::make().destination(MessageDestination::KA007)
					.DAC(MessageDAC::DAC0).channel(channel)
					.setting(MessageSetting::MOVEFREQUENCY)
					.frequencyMHz(0).amplitudePercent(0).phaseDegrees(0).instantFTW(1).ATWIncr(0).stepSequenceID(singleStepID).FTWIncr(0).phaseJump(1);;
				ms.enqueue(m);
			}
		...
		for (int channel = 0; channel < 16; channel++) {
			if (channel < nx)
			{
				...
				Message m = Message::make().destination(MessageDestination::KA007)
					.DAC(MessageDAC::DAC0).channel(channel)
					.setting(MessageSetting::MOVEFREQUENCY)
					.frequencyMHz(freq).amplitudePercent(amp).phaseDegrees(0).instantFTW(0).ATWIncr(ampstep).stepSequenceID(singleStepID).FTWIncr(freqstep).phaseJump(0);;
				ms.enqueue(m);
			}
			else
			{
				Message m = Message::make().destination(MessageDestination::KA007)
					.DAC(MessageDAC::DAC0).channel(channel)
					.setting(MessageSetting::MOVEFREQUENCY)
					.frequencyMHz(0).amplitudePercent(0).phaseDegrees(0).instantFTW(1).ATWIncr(0).stepSequenceID(singleStepID).FTWIncr(0).phaseJump(0);;
				ms.enqueue(m);
			}
		}
		...
		for (int channel = 0; channel < 16; channel++) {
			if (channel < nx) {
				...
				Message m = Message::make().destination(MessageDestination::KA007)
					.DAC(MessageDAC::DAC0).channel(channel)
					.setting(MessageSetting::MOVEFREQUENCY)
					.frequencyMHz(freq).amplitudePercent(0.002).phaseDegrees(0).instantFTW(1).ATWIncr(-ampStepMag).stepSequenceID(singleStepID).FTWIncr(0).phaseJump(0);;
				ms.enqueue(m);
